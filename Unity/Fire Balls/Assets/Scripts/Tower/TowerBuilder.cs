...
public class TowerBuilder : MonoBehaviour
{
    ...
    {
        return Instantiate(_blockTemplate, GetBuildPoint(currentBuildPoint), Quaternion.Euler(90, 0, 0), _buildPoint);
    }

    private Vector3 GetBuildPoint(Transform currentSegment)
    {
        return new Vector3(_buildPoint.position.x, 
                            currentSegment.position.y + currentSegment.localScale.y / 2 + _blockTemplate.transform.position.y / 2,
